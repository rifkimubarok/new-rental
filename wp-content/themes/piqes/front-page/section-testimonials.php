<div class="front_page_section front_page_section_testimonials<?php
	$piqes_scheme = piqes_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $piqes_scheme ) && ! piqes_is_inherit( $piqes_scheme ) ) {
		echo ' scheme_' . esc_attr( $piqes_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( piqes_get_theme_option( 'front_page_testimonials_paddings' ) );
?>"
		<?php
		$piqes_css      = '';
		$piqes_bg_image = piqes_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$piqes_anchor_icon = piqes_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$piqes_anchor_text = piqes_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( piqes_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$piqes_css      = '';
			$piqes_bg_mask  = piqes_get_theme_option( 'front_page_testimonials_bg_mask' );
			$piqes_bg_color_type = piqes_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $piqes_bg_color_type ) {
				$piqes_bg_color = piqes_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$piqes_caption = piqes_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $piqes_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $piqes_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $piqes_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$piqes_description = piqes_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $piqes_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $piqes_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $piqes_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
				...
				if ( ! piqes_exists_trx_addons() ) {
					...
					piqes_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
				}
			}
			?>
			</div>
