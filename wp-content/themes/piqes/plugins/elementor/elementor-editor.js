...
jQuery( document ).ready(
	function() {
		...
		piqes_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="piqes_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.piqes_post_editor' );
					if ( link.length > 0 ) {
						piqes_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.piqes_post_editor', function(e) {
					piqes_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('piqes_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function piqes_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
