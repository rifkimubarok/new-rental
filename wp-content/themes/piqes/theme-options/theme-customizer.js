/* global piqes_color_schemes, piqes_dependencies, Color */
...
( function( api ) {
	...
	for (var i in piqes_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'piqes-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'piqes-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'piqes-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + piqes_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( piqes_customizer_vars['msg_reset_confirm'] )) {
			...
			if (piqes_customizer_vars && piqes_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( piqes_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', piqes_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', piqes_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					piqes_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							piqes_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			piqes_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			piqes_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof piqes_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-piqes-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-piqes-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in piqes_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-piqes-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-piqes-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-piqes-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'piqes-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in piqes_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (piqes_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = piqes_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = piqes_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function piqes_customizer_get_field_value(fld) {
	...
	function piqes_customizer_check_dependencies(cont) {
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = piqes_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'piqes_options_no_use' );
					} else {
						ctrl.hide().addClass( 'piqes_options_no_use' );
					}
				}
	...
	function piqes_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( piqes_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					piqes_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in piqes_theme_fonts) {
					for (rule in piqes_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							piqes_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function piqes_customizer_actions(e) {
	...
	function piqes_customizer_update_theme_vars(opt, value) {
		piqes_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function piqes_customizer_update_theme_fonts(opt, rule, value) {
		piqes_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function piqes_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= piqes_customizer_vars['max_load_fonts']; i++) {
	...
	function piqes_customizer_update_css() {
		...
		var vars = piqes_customizer_add_theme_vars( piqes_theme_vars );

		// Make styles and add its to the css
		css += piqes_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = piqes_customizer_add_theme_fonts( piqes_theme_fonts );

		// Make styles and add its to the css
		css += piqes_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in piqes_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( piqes_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in piqes_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in piqes_color_schemes[scheme].colors) {
				colors[i] = piqes_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = piqes_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-piqes-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function piqes_customizer_add_theme_colors(colors) {
		if (piqes_additional_colors) {
			var clr = '', v = '';
			for (var k in piqes_additional_colors) {
				v   = piqes_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + piqes_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + piqes_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + piqes_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function piqes_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']   = piqes_customizer_prepare_css_value( vars['rad'] );
				...
				rez['page']              = piqes_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = piqes_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['sidebar']           = piqes_customizer_prepare_css_value( vars['sidebar'] );
				rez['gap']               = piqes_customizer_prepare_css_value( vars['gap'] );
				rez['content']           = piqes_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['gap'] );
				rez['sidebar_gap']       = piqes_customizer_prepare_css_value( vars['sidebar'] + vars['gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['gap_prc']           = vars['gap'] / vars['page'];
				rez['page_1_1']          = piqes_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = piqes_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = piqes_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = piqes_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = piqes_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = piqes_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				var margins              = [10,20,30,40,60];
				for (var m = 0; m < margins.length; m++) {
					rez['page' + margins[m]]    = piqes_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = piqes_customizer_prepare_css_value( vars['page'] - vars['gap'] - vars['sidebar'] + margins[m] );
				}
			}
		}
		return rez;
	...
	// Add ed to css value
	function piqes_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function piqes_customizer_prepare_html_value(val) {
